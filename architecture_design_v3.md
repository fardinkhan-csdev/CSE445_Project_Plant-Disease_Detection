...
- **Group-wise true integer quantization**: Frozen backbone weight `W` of shape `(C_out, C_in, k_h, k_w)` is reshaped to `(C_out, L, D_in/L)`. Each group has its own learned `scale` and `zero_point` of shape `(C_out, L)`. Weights are stored as true INT8 integers, quantized once during initialization and fixed during training (no differentiable fake-quant in forward). During forward, weights are dequantized: `W = (weight_q - zp) * scale`. This gives `L` times more quantization parameters than per-channel, increasing the quantizer's degrees of freedom while matching the paper's training protocol of true integer base weights + LoRA adapters.
...
- **Checkpoint**: Stores `weight_q` (INT8 frozen) + `scale` + `zero_point` + `lora_A` + `lora_B` + `weight_orig` (FP32 reference). At inference, weights remain INT8 `weight_q` + learned `scale`/`zero_point`. A true INT8 inference path dequantizes on-the-fly.
...
- Weights are **INT8 true integer-quantized** `[-128, 127]` (not unsigned fake-quant) — paper uses INT4; we use INT8 because Conv2d weight distributions differ from Linear
- **No explicit merge step**: at inference, weights remain INT8 `weight_q` + learned `scale`/`zero_point`. A true INT8 inference path dequantizes on-the-fly.
...
| Deviation | What Would Be Required | Mathematical Complexity | Code Effort | Research Requirement | Worth Doing |
|-----------|----------------------|------------------------|-------------|---------------------|-------------|
| **unfold+pool vs AvgPool1d** | Prove that `adaptive_avg_pool1d(unfold(x).transpose(1,2), L)` is mathematically equivalent to `AvgPool1d(x, D_in//L)` for 1×1 conv inputs, OR redesign QA to operate directly on 4D tensors without unfolding. | Very High — the paper's pooling operates on contiguous 1D feature vectors. Our unfold+pool operates on spatially-unfolded patches. Proving equivalence requires showing that the grouping of `D_in` features is identical under both operations for 1×1 conv stride/padding/dilation configurations. This is not obvious and may be false for some spatial configurations. | Medium — ~100–200 LOC for a redesigned 4D QA module if equivalence is proven. If not, ~500 LOC for a new grouping strategy. | PhD-level. This is original research into whether the paper's grouping strategy can be faithfully translated to CNNs. A negative result (proof that they are NOT equivalent) would be a valid contribution. | **Yes, for correctness.** This is the only deviation that directly affects the mathematical validity of the group-wise DOF-balancing argument. If unfold+pool is NOT equivalent to AvgPool1d, our implementation is solving a different problem than the paper. However, for 1×1 convs with stride=1 and no padding, the two operations are likely equivalent — this needs proof, not just assumption. |
| **INT4 asymmetric quantization** | Implement true INT4 asymmetric quantization with zero-point: `W_q = clamp(round(W / alpha + beta), -8, 7)`. Verify that EfficientNet-B0 Conv2d weight distributions fit in INT4 range without excessive clipping. | Medium — the quantization formula is straightforward. The challenge is statistical: Conv2d weights in EfficientNet have different distribution shapes than Transformer Linear weights. May require weight preprocessing (e.g., per-channel scaling) to make INT4 feasible. | Medium — ~50–100 LOC for INT4 quant/dequant + clipping analysis. | Medium. Would need to study weight distributions across all Q-path layers, determine if INT4 causes >1% accuracy degradation, and if so, develop preconditioning strategies. | **Yes, for true parity.** INT4 is the paper's core quantization choice. Our INT8 fallback changes the memory/accuracy tradeoff. However, if Conv2d weights genuinely cannot fit in INT4 without >1% degradation, INT8 is the correct engineering choice — making this "worth doing" conditional on empirical validation. |
| **Zero-point merge rule** | Implement `merge_with_quantization(beta, lora_A, lora_B) = beta - s * (lora_B @ lora_A).T / alpha` exactly as in Algorithm 1. Prove that merged weights remain in INT4 range. | Medium-High — the formula is simple, but the paper's correctness proof assumes Linear layer structure. For Conv2d, we must verify that the merged 4D weight tensor can still be represented in INT4 after zero-point adjustment. | Low-Medium — ~50–100 LOC for the merge function + validation. | Medium-High. The paper provides a proof sketch for Linear layers ("for any j, there exists a new zero factor β'j..."). Extending this proof to Conv2d's 4D grouped structure requires mathematical analysis. | **Yes, mandatory for true parity.** This is the paper's key innovation — the zero-point update rule is what enables post-training INT4 without FP16 fallback. Without it, we cannot produce a true INT4/INT8 checkpoint and must always store FP32 weights + learned parameters. This is the single most important missing piece. |
| ~~Fake-quant vs true integer quantization~~ | Align with the paper's actual training protocol: load a true GPTQ-quantized INT4 base model before fine-tuning, rather than using fake-quant + STE during training. Alternatively, if staying with fake-quant for CNN compatibility, document this explicitly as a CNN-specific adaptation rather than implying the paper uses the same approach. | Low for protocol alignment — the official `yuhuixu1993/qa-lora` repo uses `AutoGPTQForCausalLM.from_quantized(..., trainable=True)`, loading a true INT4 checkpoint before adapter training. No STE is used. | Low — ~10–20 LOC to switch from fake-quant to true integer quantization, plus validation that the quantized base + LoRA adapters train correctly on EfficientNet-B0 Conv2d layers. | None — this is an implementation fidelity fix, not original research. The paper's protocol is well-documented in its official repo. | **Done.** Our V3 now stores weights as true INT8 integers, quantized once during initialization and dequantized in forward with trainable scale/zp. No differentiable fake-quant in forward. This matches the paper's official protocol of true quantized base + LoRA adapters, adapted for Conv2d with INT8 instead of INT4. |
...
1. **Mathematical proof of unfold+pool equivalence**: Prove or disprove that our `F.unfold` + `adaptive_avg_pool1d` grouping is equivalent to the paper's `nn.AvgPool1d(D_in // L)` for 1×1 convolutions. If equivalent, document the proof. If not, redesign QA's grouping for 4D tensors. Effort: ~1–3 months of theoretical work.
