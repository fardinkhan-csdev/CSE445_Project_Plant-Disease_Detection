"""
V2 Base Trainer - Enhanced training pipeline with domain adaptation methods.

Supports:
- MixStyle forward hooks (Zhou et al., ICLR 2021)
- Aggressive augmentation via config
- AdaIN style mixing during training
- AdaBN test-time adaptation utilities
"""
...
from models.peft.mixstyle import MixStyle, register_mixstyle_hooks, remove_mixstyle_hooks
...
class BaseTrainerV2:
    def __init__(self, model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, 
                 config_path: str = 'config/base_config_v2.yaml', experiment_name: str = 'base_v2',
        ...
        self.use_mixstyle = bool(training_config.get('use_mixstyle', False))
        self.mixstyle_prob = float(training_config.get('mixstyle_prob', 0.5))
        self.mixstyle_layers = training_config.get('mixstyle_layers', [0, 2, 4])
        self.aggressive_augmentation = bool(training_config.get('aggressive_augmentation', False))
    ...
    def _setup_mixstyle(self):
        if self.use_mixstyle:
            mixstyle = MixStyle(p=self.mixstyle_prob)
            self.mixstyle_handles = register_mixstyle_hooks(self.model, mixstyle, self.mixstyle_layers)
            self.logger.info(f"MixStyle enabled: p={self.mixstyle_prob}, layers={self.mixstyle_layers}")
        else:
            self.logger.info("MixStyle disabled")
    
    def _cleanup_mixstyle(self):
        if self.mixstyle_handles:
            remove_mixstyle_hooks(self.mixstyle_handles)
            self.logger.info("MixStyle hooks removed")
    ...
    def load_checkpoint(self, name: str):
        ...
        if checkpoint_path is not None:
            ...
            self.run_id = checkpoint.get('run_id', self.run_id)
