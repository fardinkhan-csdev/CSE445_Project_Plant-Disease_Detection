...
def run_experiment(trainer_class, trainer_name, train_loader, val_loader, test_loader, class_info, resume=False):
    print(f"\n{'='*50}")
    print(f"Running {trainer_name} Experiment (V3)")
    print(f"{'='*50}")
...
def main(trainer_name='all', resume=False):
    ...
    if trainer_name == 'all':
        ...
        if trainer_name not in trainer_map:
            print(f"ERROR: Unknown trainer '{trainer_name}'. Valid options: lora, qlora, qalora, all")
            return
    ...
    if len(all_results) > 0:
        ...
        results_path = os.path.join('experiments', 'results', f'experiment_results_v3_{timestamp}.csv')
