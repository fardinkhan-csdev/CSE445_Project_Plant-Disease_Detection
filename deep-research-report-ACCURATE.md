...
- **Quantized LoRA (QLoRA/QA-LoRA):** In NLP, *QLoRA* quantizes a frozen model to 4-bit and backpropagates into LoRA adapters, enabling fine-tuning of 65B models on one GPU. *QA-LoRA* (Xu et al., ICLR 2024) uses **group-wise quantization** (more scale/zp params) and **grouped LoRA A** (smaller adapter matrix) to keep the model fully quantized after fine-tuning without FP16 fallback. These ideas suggest analogous approaches for CNNs (e.g. 8-bit or 4-bit EfficientNet + LoRA). No published CNN-specific QLoRA exists, but these LLM techniques highlight feasibility: e.g. QLoRA retains 99.3% of ChatGPT performance while slashing memory.  
...
11. **Xu et al., 2024 – ICLR 2024 "QA-LoRA: Quantization-Aware LoRA for LLMs".** Key idea: Unlike QLoRA which falls back to FP16 after merging, QA-LoRA keeps the model fully quantized by **balancing degrees of freedom**: it uses **group-wise quantization** (each output channel split into L groups, each with its own scale/zero-point, increasing quantization params) alongside **grouped LoRA A** (LoRA A matrix reduced from (D_in, rank) to (L, rank) by adaptive avg-pooling input to L groups, decreasing adaptation params). After fine-tuning, weights remain in INT4/INT8 without FP16 fallback. **Result:** Outperforms QLoRA+GPTQ at 2/3/4-bit on MMLU, on par with QLoRA without PTQ, but inference matches QLoRA+GPTQ speed. **Relevance:** The real algorithm is **not** "learnable per-channel scale/zero." It is **group-wise quant + grouped LoRA A**. This project's V3 implements Algorithm 1 faithfully adapted for Conv2d. *Thesis-quote:* "QA-LoRA addresses the issue by introducing group-wise operators which increase the degree of freedom of low-bit quantization (each group is quantized individually) and decrease that of LoRA (each group shares the adaptation parameters)."  
...
- **Quantized Adapters (QLoRA/QA-LoRA):** While developed for LLMs, the concept can transfer. *QLoRA* freezes a 4-bit backbone and trains LoRA matrices in fp16, achieving extreme memory savings. *QA-LoRA* (Xu et al., ICLR 2024) balances quantization and adaptation degrees of freedom via **group-wise quantization** (more scale/zp per output channel group) and **grouped LoRA A** (smaller adapter matrix via adaptive avg-pooling). The model stays fully quantized (INT4/INT8) after fine-tuning without FP16 fallback. For CNNs, one would quantize EfficientNet and insert grouped LoRA adapters in 1×1 convs. This can reduce GPU memory significantly. The tradeoff is slightly more trainable params (extra quantization params) but fewer adapter params. 
