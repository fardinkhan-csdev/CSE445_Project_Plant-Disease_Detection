...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{method_diagrams.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{qalora_training_curves.png}
\caption{Training curves for the QA-LoRA V3 run on PlantVillage. Top: training and validation cross-entropy loss per epoch. Bottom: training and validation accuracy per epoch. The validation accuracy plateaus at the test-set value within the first few epochs, consistent with the small trainable parameter count and the regularizing effect of INT4 quantization. Early stopping with patience 7 selects the checkpoint with the highest validation macro-F1.}
\label{fig:qalora_curves}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{lora_confusion_matrix.png}
\caption{Confusion matrix for LoRA V3 on the PlantVillage test set (10{,}709 images, 38 classes). The matrix is essentially diagonal; the few off-diagonal entries are concentrated in visually similar crop-disease pairs (for example, \emph{Tomato\_Septoria\_leaf\_spot} vs.\ \emph{Tomato\_Early\_blight} and \emph{Apple\_scab} vs.\ \emph{Apple\_black\_rot}). The QA-LoRA confusion matrix is qualitatively similar but with fewer off-diagonal entries in these same pairs.}
\label{fig:cm}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\columnwidth]{param_efficiency.png}
